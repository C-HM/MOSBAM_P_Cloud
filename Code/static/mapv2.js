...
async function fetchAndDisplaySites() {
  try {
    const res = await fetch("/api/sites"); // Appelle l'API
...
async function showMore(id) {
  try {
    const res = await fetch(`/api/site-details/${id}`);
    if (!res.ok) throw new Error(`Erreur HTTP: ${res.status}`);
...
async function countrySearch(country) {
  try {
    const res = await fetch(
      `/api/country-search?country=${encodeURIComponent(
        country
      )}`
    );
    if (!res.ok) throw new Error(`Erreur HTTP: ${res.status}`);
...
async function regionSearch(region) {
  try {
    const res = await fetch(
      `/api/region-search?region=${encodeURIComponent(
        region
      )}`
    );
    if (!res.ok) throw new Error(`Erreur HTTP: ${res.status}`);
...
async function fetchAndDisplayHistorique() {
  try {
    const res = await fetch("/api/sites-historique");
    if (!res.ok) throw new Error(`Erreur HTTP: ${res.status}`);
